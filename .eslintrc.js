...
const NodePlugin =
  require('../node/tools/node_modules/eslint-plugin-node-core');
NodePlugin.RULES_DIR = path.resolve('..', 'node', 'tools', 'eslint-rules');
...
Module._findPath = (request, paths, isMain) => {
  ...
  if (!r && hacks.includes(request)) {
    try {
      return require.resolve(`../node/tools/node_modules/${request}`);
    } catch {
      return require.resolve(
        `../node/tools/node_modules/eslint/node_modules/${request}`);
    }
  }
  return r;
};
...
module.exports = {
  ...
  env: {
    es2022: true,
    node: true, // Not sure why node doesn't need this.
  },
